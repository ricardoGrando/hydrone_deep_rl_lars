...
class Env():
    def __init__(self):
        ...
        self.position = Pose()
        self.pub_cmd_vel = rospy.Publisher('/hydrone_aerial/cmd_vel', Twist, queue_size=5)
        self.sub_odom = rospy.Subscriber('/hydrone_aerial/ground_truth/odometry', Odometry, self.getOdometry)
    ...
    def setReward(self, state, scan, done):
        ...
        while data is None:
            try:
                data = rospy.wait_for_message('/hydrone_aerial/scan', LaserScan, timeout=5)
            except:
                pass
        # print(data.ranges)
